...
def stops(tdf, stop_radius_factor=0.5, minutes_for_a_stop=20.0, spatial_radius_km=0.2, leaving_time=True, no_data_for_minutes=1e12, min_speed_kmh=None, cores=8):
    """Stops detection.
    ...
    if len(groupby) > 0:
        ...
        with Pool(processes=cores) as pool:
            partial_func = partial(_stops_trajectory, stop_radius=stop_radius, minutes_for_a_stop=minutes_for_a_stop, leaving_time=leaving_time, no_data_for_minutes=no_data_for_minutes, min_speed_kmh=min_speed_kmh)
            retLst = pool.imap(partial_func, [tu[1] for tu in tdf.groupby(groupby, group_keys=False, as_index=False)])
            pool.close()
            pool.join()

        stdf = pd.concat(retLst).reset_index(drop=True)
